...
def pre_measurement():
    ...
    #try :
        ...
        while True :
            ...
            dw_df = pd.DataFrame(oracle_result, columns = ["PATNO", "ORDDATE", "ORDSEQNO", "EXAMCODE", "ORDCODE", "SEX", "AGE", "MEDTIME", "ORDACTPL", "PATFG", "MEDDEPT", "ORDNAME", "SLIPCODE", "ORDDR", "CHADR", "EXECDR", "READDR1", "ACPTDT", "EXECDT", "READDT", "REPTDT", "SPCID", "EXAMTYP", "RSLTTYP", "RSLTUNIT", "RSLTNUM", "RSLTTEXT", "CONCLUSION", "COMMENTS", "NORHIGH", "NORLOW", "NORMFG", "DELTAFG", "PANICFG", "CRITHIGH", "CRITLOW", "CRITFG", "GROSRSLT", "GROSSDOC", "EXAMCNT", "ELTDATE"] )
            dw_df.fillna('None')
            dw_df.replace({np.nan:None}, inplace = True)

            # 데이터프레임은 null은 None으로 읽음. 별도 처리 없이 DB에 insert하는 경우 문자 None으로 insert되어 null로 변경하기 위한 처리
            ...
            dw_df["RSLTTEXT"] = dw_df["RSLTTEXT"].apply(none_to_lob)
            dw_df["CONCLUSION"] = dw_df["CONCLUSION"].apply(none_to_lob) 
            dw_df["GROSRSLT"] = dw_df["GROSRSLT"].apply(none_to_lob)

            # 데이터프레임에서 ((values, val), (values, val), ...) 이런 형태로 변경하기 위함
            dw_df = dw_df.values.tolist()

            sql = """
                insert into cdmpv531.pre_measurement
                (patno, orddate, ordseqno, examcode, ordcode, sex, age, medtime, ordactpl, patfg, meddept, ordname, slipcode, orddr, chadr, execdr, readdr1, acptdt, execdt, readdt, reptdt, spcid, examtyp, rslttyp, rsltunit, rsltnum, rslttext, conclusion, "comments", norhigh, norlow, normfg, deltafg, panicfg, crithigh, critlow, critfg, grosrslt, grossdoc, examcnt, eltdate)
                values %s;
                """
