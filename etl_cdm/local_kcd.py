...
def local_kcd():
    ...
    try :
        ...
        fetch_size = 10000
        # DW Oracle에서 조회
        table_name = "CUHDW.ODS_SIDIAGCT"
        dw_table_name = "CUHDW.DD_SIDIAGCT"
        oracle_sql = """
                            select
                                a.DIAGCODE
                                , a.FROMDATE 
        ...
        # int타입에 null값이 있는 경우 None으로 변경하기 위함 
        local_kcd_df["concept_id"] = local_kcd_df["concept_id"].fillna(-1).astype('int16').replace({-1: None})
